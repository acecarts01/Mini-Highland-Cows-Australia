...
export function GET() {
  ...
  const lines = [
    ...
    '',
    '## Pages worth citing',
    `- [Homepage](${absoluteUrl('/')}): brand overview and featured animals.`,
    `- [Full available herd](${absoluteUrl('/herd')}): live-availability list, updated as animals sell.`,
    `- [Shop / filterable catalog](${absoluteUrl('/shop')}): livestock and care equipment, filterable by size, colour, Chondro status.`,
    `- [FAQ](${absoluteUrl('/faq')}): PIC, NLIS, acreage, Chondro, delivery and payment questions answered.`,
    `- [Chondrodysplasia genetics guide](${absoluteUrl('/chondro-guide')}): carrier vs non-carrier explained, DNA testing policy.`,
    `- [About / business registration](${absoluteUrl('/about')}): ABN, location, biosecurity policy.`,
    '',
    '## Guides',
    ...BLOG_POSTS.map((p) => `- [${p.title}](${absoluteUrl(`/blog/${p.slug}`)}): ${p.excerpt}`),
    '',
